...
#     vl.list <- expand.grid(v = 1:nrow(variables), l = 1:length(land_grid_list))
    burn.out <- lapply(seq(nrow(variables)), function(v){
#     for(v in 1:nrow(variables)){
        ...
        centroids <- land_grid_list[[variables[[v,'land']]]]$centroids
        grid <- land_grid_list[[variables[[v,'land']]]]$grid
        ...
        ## this piece could also be targets-ed to separate it out
